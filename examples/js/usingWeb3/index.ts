...
const MachineStationFactoryContractAddress: string =
  "0x11f40Af48D731d0f473A382DcceDd64B1f2f15a3"; // Replace with the your dedicated machine station factory contract address
...
class MachineStationFactoryExample {
  async submitDeployMachineSmartAccountTx() {
    ...
    globalMachineAddress = await this.deployMachineSmartAccount(
      machineOwner,
      nonce,
      deploySignature
    );
  ...
  async submitMachineTransferBalanceTx() {
    ...
    const machineOwnerSignature =
      await this.machineOwnerSignTypedDataTransferMachineBalance(
        machineAddress,
        recipientAddress,
        nonce
      );
    const ownerSignature = await this.ownerSignTypedDataTransferMachineBalance(
      machineAddress,
      recipientAddress,
      nonce
    );
  ...
  async submitMachineStorageBatchTx() {
    try {
      ...
      for (let index = 0; index < targets.length; index++) {
        ...
        const machineOwnerSignature =
          await this.machineOwnerSignTypedDataExecuteMachineBatch(
            machineAddresses[index],
            targets,
            calldata,
            machineNonce
          );
      ...
      const ownerSignature =
        await this.ownerSignTypedDataExecuteMachineBatchTransactions(
          machineAddresses,
          targets,
          calldata,
          nonce,
      ...
      await this.executeMachineBatchTransactions(
