...
contract MachineSmartAccount is EIP712, AccessControl, ReentrancyGuard {
    ...
    bytes32 private constant TRANSFER_BALANCE_TYPEHASH =
        keccak256(
            "TransferMachineBalance(address recipientAddress,uint256 nonce)"
        );
    ...
    function execute(
        ...
        if (!success) {
            revert Errors.TargetCallFailedV2(target, data);
        }
    ...
    function executeBatch(
        ...
        if (targets.length > Constants.MAX_BATCH_TRANSACTIONS) {
            revert Errors.MaxBatchTransactionExceeded(
                Constants.MAX_BATCH_TRANSACTIONS,
                targets.length
            );
        }
        ...
        for (uint256 i = 0; i < targets.length; i++) {
            ...
            emit Events.MachineBatchTransactionExecuted(
                address(this),
                i,
                success
            );
    ...
    function transferMachineBalance(
        ...
        uint256 nonce,
        bytes calldata signature
    ) external nonReentrant onlyRole(MACHINE_STATION_ROLE) {
